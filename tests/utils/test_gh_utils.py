...
class TestRunGhCommand:
    ...
    def test_success_json_flag_but_non_json_output(
        ...
        assert (
            found_warning
        ), "Expected 'Failed to parse JSON output' warning not printed"
    ...
    def test_file_not_found_error(self, mocker: "MockerFixture") -> None:
        ...
        When: run_gh_command is called
        Then: An error dictionary should be returned indicating the command was
              not found
        """
        # Given
        mocker.patch.dict(os.environ, {"GH_TOKEN": MOCK_TOKEN}, clear=True)
    ...
    def test_run_gh_command_generic_exception(self, mocker: MockerFixture) -> None:
